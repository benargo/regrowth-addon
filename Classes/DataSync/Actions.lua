...
local CommActions = {
    [Actions.nex] = function(Message)
        Regrowth.Utils.Messaging:success("Message received: " .. Message.content);
    end,
    [Actions.handlereceiveddata] = function(Message)
        if not Message.sender then
            Regrowth.Utils.Messaging:error("Received message from unknown sender. Ignoring.");
            return;
        end
        ...
        if not ok then
            Regrowth.Utils.Messaging:error("Failed to apply synced data from '" .. Message.sender .. "'. (" .. tostring(appliedCount) .. ")");
            return;
        end
        ...
        if not appliedCount or appliedCount == 0 then
            ...
            Regrowth.Utils.Messaging:debug("Sync from '" .. Message.sender .. "' had nothing newer to apply.");
            return;
        end
        ...
        Regrowth.Utils.Messaging:success("Received updated loot council data from '" .. Message.sender .. "'." .. elapsedText ..
